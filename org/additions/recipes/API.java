...
public class API extends Object_Identification {
	...
	private void RegisterRecipe() throws FileNotFoundException {
		...
		if (Working) {
			...
			MyRecipe.shape("abc", "def", "ghi"); // <--- Устанавливаем форму крафта
			final String tempArray[][] = {{"a", "b", "c"}, {"d", "e", "f"}, {"g", "h", "i"}}; // <--- Просто константы
			
			int logicCounter = 0; // <--- Временная переменная
				...
				for (int j = 0; j < 3; j++) {
					try {
						...
						if (this.solveItem(ItemRecipe.get(i).toString().split(" ")[j], fileList).getType() == Material.AIR)
							logicCounter++; // <--- Считаем если ингридиент будет равен пустоте
					} catch (Exception e) { 
						this.ErrorDataCollection.clear(); // <--- Выгружаем предупреждения
						this.ErrorDataCollection.add(this.FileName + " --> " + plugin.getConfig().getString("messages.item_recipe_not_found").replace("&", "§"));
						Working = false; // <--- Указываем что рецепт не работает
						break pointer; // <--- Ломаем цикл по поинтеру, чтобы не затягивать работу плагина (экономим bruh)
					}
				}
			}
			if (logicCounter == 9) { // Если рецепт не состоит из предметов, а только из пустоты то отключаем крафт
				this.ErrorDataCollection.clear(); // <--- Выгружаем предупреждения
				this.ErrorDataCollection.add(this.FileName + " --> " + plugin.getConfig().getString("messages.item_recipe_not_found").replace("&", "§"));
				Working = false; // <--- Указываем что рецепт не работает
			}
			...
			if (Working) {
				plugin.getServer().addRecipe(MyRecipe); // <--- Регистрируем новый крафт на сервер
				this.ImportData(); // <--- Загружаем данные предмета/права на крафт/рецепт и прочее
				if (ErrorDataCollection.isEmpty())
					ExtractSuccess(); // <--- Выводим что всё хорошо и предупреждений нет
				else
					ExtractWarnings(); // <--- Выводим данные из ErrorDataCollection в виде предупреждений
			}
		}
		
		if (!Working) {
			this.ErrorDataCollection.add(this.FileName + " --> " + plugin.getConfig().getString("messages.api_error").replace("&", "§"));
			ExtractErrors(); // <--- Выводим данные из ErrorDataCollection ввиде ошибок
		}
		
		this.ImportAnotherData(); // <--- Загружаем данные о работоспособности крафта в хранилище
		this.ErrorDataCollection.clear(); // <--- Выгружаем ошибки/предупреждения
	}
